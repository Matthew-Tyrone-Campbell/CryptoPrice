...
class CryptoWidget(Frame):
    def __init__(self,cryptocurrency,screen,**kwargs):
        ...
        self.logo_label = Label(master=self,image=self.logo_image,padx=self.x_padding)
        self.logo_label.grid(row= 0, column= 0)
        
        #crypto name
        self.crypto_name_frame = Frame(self,padx=self.x_padding)
        self.crypto_name_frame.grid(row=0,column=1)
        self.crypto_name = Label(master=self.crypto_name_frame,text=cryptocurrency,pady=self.y_padding)
        self.crypto_name.grid(row=0,column=0)

        self.crypto_initials = Label(master=self.crypto_name_frame,text=crypto.get_crypto_initials(cryptocurrency),pady=self.y_padding)
        self.crypto_initials.grid(row=1, column=0)

        #crypto price
        self.crypto_price = Label(master=self,text=f"${crypto.get_crypto_price(cryptocurrency)}",padx=self.x_padding)
        self.crypto_price.grid(row=0,column=2)

        #change in crypto price
        self.change_in_price = Label(master=self,padx=self.x_padding,text=f"{crypto.get_crypto_change(cryptocurrency)}%")
