...
class CryptoWidget(Frame):
    def __init__(self,cryptocurrency,screen,**kwargs):
        ...
        self.crypto_name = Label(master=self.crypto_name_frame,text=cryptocurrency,pady=y_padding,font=(font_family,font_size))
        self.crypto_name.grid(row=0,column=0)

        self.crypto_initials = Label(master=self.crypto_name_frame,text=crypto.get_crypto_initials(cryptocurrency),pady=y_padding,font=(font_family,font_size))
        self.crypto_initials.grid(row=1, column=0)

        #crypto price
        self.crypto_price = Label(master=self,text=f"${crypto.get_crypto_price(cryptocurrency)}",padx=x_padding,font=(font_family,font_size))
        self.crypto_price.grid(row=0,column=2)

        #change in crypto price
        self.change_in_price = Label(master=self,padx=x_padding,text=f"{crypto.get_crypto_change(cryptocurrency)}%",font=(font_family,font_size))
